...
def geezit_corpus(chunksz=80, type='phon', drop_propers=True, drop_addit=True, min_proper_len=4):
    ...
    for auth,title,fn in WORKS:
        ...
        if title=='Punica' and drop_addit:
            ...
            shifted_indices = np.arange(last_good_chunk, len(puni_chunks))*chunksz + chunksz
            puni_refs = ["%d:%d" % tupl for tupl in utils.indices_to_bookref(soup, np.concatenate((good_indices,shifted_indices)))]
        ...
        tmpdf = pd.DataFrame()
        try:
            chunks = string_chunks(ll, chunksz, type, drop_propers, min_proper_len=min_proper_len)
        ...
        tmpdf['Author'] = auth
        tmpdf['Work'] = title
        tmpdf['Bookref'] = ["%d:%d" % tupl for tupl in utils.indices_to_bookref(soup, (np.arange(len(chunks))*chunksz)+1)]
        df = df.append(tmpdf).reset_index(drop=True)
